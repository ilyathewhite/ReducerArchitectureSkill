...
- When `<Feature>.swift` grows because `StoreState` logic dominates it, extract that state-specific code into `<Feature>State.swift` in the same feature folder under `extension Feature.StoreState`. This is the preferred home for a large `StoreState` initializer, derived state, mutating helpers, and state-specific helper methods.
